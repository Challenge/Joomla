...
function listEvents() {
	...
	try {
		...
		$result = $stmt->fetchAll();
                ...
                if(strpos(getServerURI(), '&') !== false) {
                    $pathPrefix = getServerURI() . '&';
                } else {
                    $pathPrefix = getServerURI() . '?';
                }
		...
		foreach ($result as $row) {
			...
			echo '<a href="' . $pathPrefix . 'action=view&id=' . $id . '"><h3>Event: ' . $name . '</h3></a>', PHP_EOL;
...
function viewEvent($error = false, $table_taken = false) {
	...
	echo '<br /><br /><form action="' . getServerPath() . '" method="post" enctype="multipart/form-data">
...
<?php
$GLOBALS['dbcon'] = $this->getDatabaseConnection();
	...
	if(empty($_POST['real_name']) || (empty($_POST['table_number']) || !is_numeric($_POST['table_number'])) || empty($_POST['user_email'])) {
		...
		if(!empty($email)) {
			...
			if(!$mailSent) {
				...
				echo '<p>Hvis der ikke kommer ydeligere fejlmeddelser er din registering stadig blevet godkendt, men du skal dog stadig sikre dig at dikulan gruppen stadig f&aring;r dette at vide.<p>', PHP_EOL;
			}
		}
		...
		echo '<a href="' . getServerPath() . '"><p>G&aring; tilbage til oversigten<p></a>', PHP_EOL;
	}
}
