...
function listRegistrations($id) {
	...
	echo '<form name="registrations" action="' . getServerPath() . '" method="post" enctype="multipart/form-data">', PHP_EOL;
...
function listEvents() {
	...
	try {
		...
		$result = $stmt->fetchAll();
                ...
                if(strpos(getServerPath(), '&') !== false) {
                    $pathPrefix = getServerPath() . '&';
                } else {
                    $pathPrefix = getServerPath() . '?';
                }
		...
		foreach ($result as $row) {
			...
			echo '<a href="' . $pathPrefix . 'action=edit&id=' . $id . '"><h3>Rediger: ' . $name . '</h3></a>', PHP_EOL;
		...
		echo '<br />', PHP_EOL;
		echo '<a href="' . $pathPrefix . 'action=add"><p>Tilf&oslash;j en ny event</p></a>', PHP_EOL;
	}
	catch (PDOException $e) {
		echo "Could not establish connection to the database.";
		echo "This is unfortunately a fatal error that cannot be ignored and further execution has been halted." . "<br />";
...
function editEvent($errors = false) {
	...
	echo '<form action="' . getServerPath() . '" method="post" enctype="multipart/form-data">', PHP_EOL;
